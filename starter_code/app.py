...
class Show (db.Model):
    ...
    def __repr__(self) -> str:
        return f'Show ID: {self.id}, Venue: {self.venue.name} ({self.venue.id}), Artist: {self.artist.name} ({self.artist.id})'

    def __str__(self) -> str:
        return f'Artist {self.artist.name} performing at {self.venue.name} in {self.venue.city},{self.venue.state}'
...
def show_venue(venue_id):
    ...
    if current_venue:
        ...
        #flash('An error occurred. The selected Venue does not exist.')
        return render_template('pages/home.html')
#  Create Venue
#  ----------------------------------------------------------------
...
def create_venue_submission():
    ...
    # print(
    #     f'name: {name}, city: {city}, state: {state}, address: {address}, phone: {phone}')
    # print(
    #     f'genres: {genres} (and genre type is: {type(genres)}), facebook: {facebook_link}, image_link: {image_link}, website_link: {website_link}')
    # print(
    #     f'seeking_talent: {seeking_talent}, seeking_description: {seeking_description}')
...
def delete_venue(venue_id):
    ...
    try:
        ...
        delete_response['message'] = f'An error has occured. {name} has not been deleted.'
    finally:
        db.session.close()
        # flash(delete_response['message'])
...
def show_artist(artist_id):
    ...
    data = {
        ...
        'past_shows': [],
        'upcoming_shows': [],
        'past_show_count': len(list(filter(lambda x: x.start_time < datetime.today(), artist.shows))),
        'upcoming_shows_count': len(list(filter(lambda x: x.start_time > datetime.today(), artist.shows)))
    }
...
def create_show_submission():
    ...
    try:
        ...
        db.session.add(
            Show(venue_id=venue_id, artist_id=artist_id, start_time=start_time))
        db.session.commit()
        success = True
    except Exception as e:
        print(e)
